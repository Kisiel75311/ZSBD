...
CREATE BITMAP INDEX bmp_idx_products_name ON PRODUCTS (NAME)
-- Indeks bitmapowy na klucz obcy CONTRACTOR_FK
CREATE BITMAP INDEX bmp_idx_products_contractor_fk ON PRODUCTS (CONTRACTOR_FK)
-- Indeks bitmapowy na długość specyfikacji (jeśli zastosowanie funkcji jest możliwe w indeksie bitmapowym)
CREATE BITMAP INDEX bmp_idx_products_spec_length ON PRODUCTS (LENGTH(SPECIFICATION))

-- Indeksy bitmapowe na DOCUMENTS
-- Indeks bitmapowy na datę dokumentu
CREATE BITMAP INDEX bmp_idx_documents_date ON DOCUMENTS (DOCUMENT_DATE)
-- Indeks bitmapowy na klucz obcy CONTRACTOR_FK
CREATE BITMAP INDEX bmp_idx_documents_contractor_fk ON DOCUMENTS (CONTRACTOR_FK)
-- Indeks bitmapowy na klucz obcy CLIENT_FK
CREATE BITMAP INDEX bmp_idx_documents_client_fk ON DOCUMENTS (CLIENT_FK)
-- Indeks bitmapowy na klucz obcy DOCUMENT_TYPE_FK
CREATE BITMAP INDEX bmp_idx_documents_type_fk ON DOCUMENTS (DOCUMENT_TYPE_FK)

-- Indeksy bitmapowe na USERS
-- Indeks bitmapowy na klucz obcy WAREHOUSE_FK
CREATE BITMAP INDEX bmp_idx_users_warehouse_fk ON USERS (WAREHOUSE_FK)

-- Indeksy bitmapowe na PRICE_LISTS
-- Indeks bitmapowy na datę ważności cennika
CREATE BITMAP INDEX bmp_idx_price_lists_valid_date ON PRICE_LISTS (VALID_DATE)

-- Dodatkowe indeksy bitmapowe
-- Złożony indeks bitmapowy na CLIENT_FK i DOCUMENT_DATE w DOCUMENTS
CREATE BITMAP INDEX bmp_idx_documents_client_fk_date ON DOCUMENTS (CLIENT_FK, DOCUMENT_DATE)
...
CREATE BITMAP INDEX bmp_idx_products_contractor_fk_length ON PRODUCTS (CONTRACTOR_FK, PRODUCT_LENGTH)
...
CREATE BITMAP INDEX bmp_idx_price_lists_product_fk_sale ON PRICE_LISTS (PRODUCTS_FK, SALE)
...
CREATE BITMAP INDEX bmp_idx_product_documents_doc_fk_amount ON PRODUCT_DOCUMENTS (DOCUMENT_FK, AMOUNT)

-- Indeks bitmapowy na PRODUCTS.NAME dla porównań z LIKE '%a%'
CREATE BITMAP INDEX bmp_idx_func_products_name ON PRODUCTS (LOWER(NAME))

-- Indeks bitmapowy na rok z DOCUMENTS.DOCUMENT_DATE
CREATE BITMAP INDEX bmp_idx_func_documents_year ON DOCUMENTS (EXTRACT(YEAR FROM DOCUMENT_DATE))
...
CREATE BITMAP INDEX bmp_idx_func_documents_date_range ON DOCUMENTS (CASE WHEN DOCUMENT_DATE BETWEEN TO_DATE('2020-01-01', 'YYYY-MM-DD') AND TO_DATE('2022-12-31', 'YYYY-MM-DD') THEN 1 ELSE 0 END)
